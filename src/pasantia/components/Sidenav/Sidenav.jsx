...
export const Sidenav = ({...rest}) => {
  const {miniSidenav, transparentSidenav, whiteSidenav, darkMode, sidenavColor, setMiniSidenav, setTransparentSidenav, setWhiteSidenav} = useContext( CustomTheme );
  ...
  const renderRoutes = routes.map(({ type, name, icon, title, key, route, items}) => {
     let returnValue;
     if (type === "collapse") {
       returnValue = (
